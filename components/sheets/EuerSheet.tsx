...
export function EuerSheet({ userId }: { userId: string }) {
  ...
  /* ── Summary: EUeR ───────────────── */
  const totalExpNet = expenses.reduce((s, r) => s + (r.net_amount ?? 0), 0);
  const totalIncNet = income.reduce((s, r) => s + (r.net_amount ?? 0), 0);
    ...
    <div className="card flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 dark:bg-gray-800">
      <div className="flex items-center gap-6 text-sm">
        <div>
          ...
          <span className="ml-2 font-semibold">{fmtEUR(totalIncNet)}</span>
        </div>
        <span className="text-muted">-</span>
        <div>
          <span className="text-muted">Ausgaben Netto</span>
          <span className="ml-2 font-semibold">{fmtEUR(totalExpNet)}</span>
        </div>
        <span className="text-muted">=</span>
        <div>
          <span className="text-muted">Gewinn</span>
          <span className={`ml-2 font-bold text-base ${gewinn >= 0 ? 'text-accent' : 'text-red-600'}`}>
            {fmtEUR(gewinn)}
          </span>
        </div>
      </div>
